...
//        }
        ...
        for (int i = 0; i < result.size(); i++) {
            ...
            StringBuilder mics=new StringBuilder();
            mics.append(result.get(i).getTime()+";"+result.get(i).getLng()+";"+result.get(i).getLat());
            for (int j = 3; j < names.split(";").length; j++) {
                Integer index = getIndex(values, names.split(";")[j]);
                mics.append(";"+values.get(index).getValue());
            }
            datas.add(mics.toString());
        }
        return datas;
    ...
    public static Integer getIndex(List<DensityVo.KV> values,String name){
...
//            datas=searchStrMic(densityDto);
            datas=searchStrMic(densityDto,zhCollectRecord.getRemark());
        }
        zhCollectRecordDto.setDatas(datas);
//        List<DensityVo> densityVos = searchMic(densityDto);
